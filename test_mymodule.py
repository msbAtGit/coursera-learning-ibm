...
class TestAdd(unittest.TestCase):
    """
    Testing the add method in mymodule
    """
    def test_add(self):
        """
        Having some assertions to test the methods
        """

        self.assertEqual(add(9, 4), 13)

        #Checking for negative addition
        self.assertEqual(add(-10, -12), -22)
        self.assertEqual(add(2, 4), 6)
        self.assertEqual(add("Hello", "World"), "HelloWorld")
        self.assertEqual(add(2.3, 5.9), 8.2)
        self.assertNotEqual(add(-2,-2), 0)
        self.assertEqual(add(2.3000, 5.9000), 8.2)
